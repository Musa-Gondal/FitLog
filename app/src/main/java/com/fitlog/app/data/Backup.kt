...
object Backup {
    ...
    suspend fun export(db: AppDatabase, profile: Profile): String {
        ...
        root.put("profile", JSONObject().apply {
            ...
            put("restSeconds", profile.restSeconds); put("ratePct", profile.ratePct)
            put("adaptiveEnabled", profile.adaptiveEnabled); put("adaptiveTdee", profile.adaptiveTdee)
            put("lastCheckIn", profile.lastCheckIn); put("keepAwake", profile.keepAwake); put("barKg", profile.barKg)
        })
        ...
        val notesByWorkout = db.workout().allNotes().groupBy { it.workoutId }
            ...
            db.workout().allWorkouts().forEach { w ->
                ...
                setsByWorkout[w.id].orEmpty().forEach {
                    sets.put(JSONObject().put("ex", it.exercise).put("eo", it.exerciseOrder)
                        ...
                        .put("type", it.setType).put("rpe", it.rpe ?: JSONObject.NULL))
                }
                val notes = JSONArray()
                notesByWorkout[w.id].orEmpty().forEach { notes.put(JSONObject().put("ex", it.exercise).put("note", it.note)) }
                put(JSONObject().put("name", w.name).put("start", w.startMillis).put("end", w.endMillis)
                    .put("notes", w.notes).put("sets", sets).put("exNotes", notes))
            }
        })
    ...
    suspend fun import(db: AppDatabase, json: String, current: Profile): Profile {
        ...
        db.withTransaction {
            ...
            db.workout().clearWorkouts(); db.workout().clearSets(); db.workout().clearNotes()
                ...
                for (i in 0 until arr.length()) {
                    ...
                    w.optJSONArray("sets")?.objects {
                        sets += WorkoutSet(workoutId = id, exercise = it.getString("ex"), exerciseOrder = it.getInt("eo"),
                            ...
                            setType = it.optString("type", "N").ifBlank { "N" },
                            rpe = if (it.isNull("rpe")) null else it.optDouble("rpe"))
                    }
                    db.workout().insertSets(sets)
                    val notes = mutableListOf<ExerciseNote>()
                    w.optJSONArray("exNotes")?.objects {
                        notes += ExerciseNote(workoutId = id, exercise = it.getString("ex"), note = it.getString("note"))
                    }
                    db.workout().insertNotes(notes)
                }
            }
        }
        ...
        return current.copy(
            ...
            ratePct = p.optDouble("ratePct", current.ratePct),
            adaptiveEnabled = p.optBoolean("adaptiveEnabled", current.adaptiveEnabled),
            adaptiveTdee = p.optInt("adaptiveTdee", current.adaptiveTdee),
            lastCheckIn = p.optString("lastCheckIn", current.lastCheckIn),
            keepAwake = p.optBoolean("keepAwake", current.keepAwake),
            barKg = p.optDouble("barKg", current.barKg),
        )
    }
}
