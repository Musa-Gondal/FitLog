...
fun Ring(progress: Float, color: Color, modifier: Modifier = Modifier, size: Dp = 150.dp, stroke: Dp = 14.dp,
    ...
    val anim by androidx.compose.animation.core.animateFloatAsState(
        progress, androidx.compose.animation.core.tween(900), label = "ring",
    )
    Box(modifier.size(size), contentAlignment = Alignment.Center) {
        Canvas(Modifier.size(size)) {
            ...
            val sweep = 360f * anim.coerceIn(0f, 1f)
...
fun MacroBar(label: String, value: Double, target: Int, color: Color, unit: String = "g") {
    Column(Modifier.fillMaxWidth().padding(vertical = 4.dp)) {
        ...
        val anim by androidx.compose.animation.core.animateFloatAsState(
            ...
            androidx.compose.animation.core.tween(700), label = "bar",
        )
...
fun LineChart(points: List<Pair<String, Float>>, modifier: Modifier = Modifier, color: Color = MaterialTheme.colorScheme.primary,
              unit: String = "", overlay: List<Float>? = null, overlayColor: Color = MaterialTheme.colorScheme.tertiary,
              target: Float? = null) {
    if (points.size < 2) {
    ...
    val all = points.map { it.second } + overlay.orEmpty() + listOfNotNull(target)
    val min = all.min()
    val max = all.max()
    ...
    Column(modifier) {
        ...
        Canvas(Modifier.fillMaxWidth().height(160.dp)) {
            ...
            if (target != null) {
                val ty = pad + h - h * (target - min) / range
                drawLine(grid.copy(alpha = 0.9f), Offset(pad, ty), Offset(size.width - pad, ty), strokeWidth = 1.5.dp.toPx(),
                    pathEffect = androidx.compose.ui.graphics.PathEffect.dashPathEffect(floatArrayOf(12f, 10f)))
            }
            if (overlay != null && overlay.size == points.size) {
                // Raw values as faint dots, overlay (e.g. trend) as the bold line.
                pts.forEach { drawCircle(color.copy(alpha = 0.45f), 3.dp.toPx(), it) }
                val op = overlay.mapIndexed { i, v -> Offset(pad + w * i / (points.size - 1), pad + h - h * (v - min) / range) }
                val tp = Path().apply { moveTo(op.first().x, op.first().y); op.drop(1).forEach { lineTo(it.x, it.y) } }
                drawPath(tp, overlayColor, style = Stroke(3.dp.toPx(), cap = StrokeCap.Round))
            } else {
